...
COREDEF="00_core"
RESTORECON="${ROOT_DIR}/sbin/restorecon"
...
create_file() {
	EXEC="
	touch \"$1\";
	[ -x ${RESTORECON} ] && ${RESTORECON} \"$1\" >/dev/tty 2>&1;
	chown ${TUSER}.${TGROUP} $1 || echo \"Failed to set owner -${TUSER}- for -$1-.\" >/dev/tty 2>&1;
	chmod ${TMODE} $1 || echo \"Failed to set mode -${TMODE}- for -$1-.\" >/dev/tty 2>&1 "
